...
- **Estrutura Clean Architecture:** Projeto organizado em camadas (`cmd`, `internal`, `pkg`, `test`).
- **Entidades de domínio:** `User` e `Transaction` implementadas com regras de negócio e validação.
- **DTOs e conversores:** Prontos para uso entre camadas.
- **Testes unitários:** Presentes para entidades principais.
- **Servidor HTTP:** Implementado com Gin, endpoints básicos criados (`/api/v1/health`, `/api/v1/info`, `/api/v1/users/`, `/api/v1/transactions/`).
- **Configuração de ambiente:** `.env` pronto para uso local, com variáveis para banco, porta e serviços externos.
- **Migrations SQL:** Scripts para criar e popular tabelas no PostgreSQL.
- **Documentação de setup:** `setup-dev.md` com instruções detalhadas para rodar localmente e com Docker.
- **Docker Compose:** Pronto para subir banco e app (quando Docker estiver funcional).
- **Validação de CPF/CNPJ, saldo, tipos de usuário:** Implementada nas entidades.
- **Health check e info endpoint:** Disponíveis para verificação rápida do serviço.

---

## ❌ O que ainda falta

- **Repository Layer:** Implementar interfaces e repositórios concretos para persistência no PostgreSQL (CRUD de usuários, transações).
- **UseCase Layer:** Implementar casos de uso (ex: transferência, consulta de saldo, etc).
- **Handler Layer:** Implementar endpoints REST completos (`POST /transfer`, etc), recebendo e respondendo com DTOs.
- **Integração com serviços externos:**
	- Chamada ao mock de autorização antes de transferir (`https://util.devi.tools/api/v2/authorize`).
	- Chamada ao mock de notificação após transferência (`https://util.devi.tools/api/v1/notify`).
- **Transação atômica:** Garantir rollback em caso de falha na transferência.
- **Testes de integração:** Para endpoints e fluxo de transferência.
- **Tratamento de erros e respostas HTTP padronizadas.**
- **Documentação dos endpoints:** (Swagger ou README).
- **Middleware de autenticação/autorização:** (opcional, diferencial).
- **Ajustes finais de Docker:** (quando resolver problemas do Docker Desktop).
- **Cobertura de testes consistente:** (diferencial).
- **Proposta de melhorias arquiteturais:** (diferencial).
