...
func TestNewUser(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			user, err := NewUser(tt.fullName, tt.document, tt.email, tt.password, tt.userType)
...
func TestUser_HasSufficientBalance(t *testing.T) {
	user, _ := NewUser("João Silva", "11144477735", "joao@teste.com", "123456", UserTypeCommon)
